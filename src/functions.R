...
detection_function = function(x,L=1,x0=10,k=-0.3){
  y = L/(1+exp(-1*k*(x-x0))) 
  return(y)
}

check_detection_function = function(rmax = 25, L=1,x0=10,k=-0.3){
  xx = seq(from = 0, to = rmax, by = 0.1) # range vector
  plot(xx, detection_function(x = xx, L = L, x0 = x0, k = k), xlab = 'R', ylab = 'P', type = 'l')
}

pick_rws = function(nrws=1e3,lim=25){
  ...
  while(cnt <= nrws){
    ...
    tmp = rbinom(1, size = 1, prob = detection_function(x = irng, x0 = 5, k = -1))
  ...
  # prep output
  out = data.frame(x0=XX, y0=YY,rng=RNG)
  return(out)
}

get_detection_fxn = function(nrws = 1e3, platform = 'acoustic', debug = FALSE){
  ...
  if(platform == 'acoustic'){
    ...
    # plot
    if(debug){
      plot(xx,yy, xlab = 'Range [km]', ylab = 'P')
    }
    
  } else if(platform == 'visual'){
    
    rngs = runif(n = nrws,min = 0, max = 100)
...
rw_sims = function(nrws = 100, 
  ...
  # get detection functions
  rngs = get_detection_fxn(nrws = nrws, platform = platform, debug = debug)
  
  # calculate starting point within circle
  a=2*pi*runif(n = nrws,0,1)
  x0=rngs*cos(a)
  y0=rngs*sin(a)
  ...
  for(jj in 1:nrws){
    ...
    tmp = rw_sim(x0 = x0[jj], y0 = y0[jj], hrs = hrs, bh = bh, nt=nt,td = td)
